...
logger.add(
    "log_folder/bangla_asr_{time:YYYY-MM-DD}.log",
...
# Initialize the Whisper model and processor (BanglaASR from HuggingFace)
...
def save_audio(audio_data, filename='default'):
    if filename=='default':
        filename = f'{SAVE_DIR}/EC_AUDIO_DATA_whisper_' + datetime.now().strftime('%Y-%m-%d_%H-%M-%S') + '.wav'
...
def transcribe_audio(audio_data):
    """Transcribe single audio data using Whisper model."""
    # Process the audio data with the feature extractor
    input_features = feature_extractor(audio_data, sampling_rate=RATE, return_tensors="pt").input_features.to(device)
